...
Capriole AI provides one account and API key across supported model families, but it does not flatten every provider into one imaginary protocol. It maintains several public paths: OpenAI Responses, OpenAI-compatible Chat Completions, Anthropic Messages, and a simple Capriole-native chat endpoint.

The result is unified access without pretending the underlying APIs are identical.
...
That boundary matters. Once a streamed response has begun, switching sources would risk duplicated or contradictory output. Automatic fallback is not the same as choosing a different model halfway through a response, and it is not a promise that every upstream failure can be hidden.
...
This distinction avoids an impossible promise. A casual browser conversation and an automated workload processing millions of tokens should not share an invisible unlimited allowance.
