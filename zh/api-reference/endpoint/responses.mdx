...
`POST /v1/responses` 接受 `openai-latest`、`openai/gpt-6-astra`、`openai/gpt-5.6-terra`、`openai/gpt-5.6-luna` 和 `openai/gpt-5.5`。使用 `openai-latest` 可以让 Capriole AI 选择当前推荐的 OpenAI Responses 旗舰模型。

也可以发送不带 `openai/` 前缀的原生 OpenAI ID，例如 `gpt-6-astra`。原生 ID 与带前缀的 ID 使用同一模型记录进行路由、计价和额度扣减。
