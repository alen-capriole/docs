...
Capriole AI web chat and the public API are separate product surfaces. In web chat, Fable 5 and Fable 5 Thinking are the primary Anthropic modes, while Opus 5 and Opus 5 Thinking appear under Other Models. The public API does not expose web chat thinking presets as separate model IDs; for Claude Chat API requests, use `claude-latest` or a concrete public Claude model ID such as `anthropic/claude-fable-5`, `anthropic/claude-opus-5`, or `anthropic/claude-sonnet-4-6`. Existing Opus 4.8, Opus 4.7, and Opus 4.6 integrations remain supported.
