...
`POST /v1/chat` and `POST /v1/chat/completions` support all three latest aliases. `POST /v1/responses` supports `openai-latest`. `POST /v1/messages` supports `claude-latest` plus concrete Claude Messages IDs such as `claude-fable-5`, `claude-opus-5`, `claude-opus-4-8`, `claude-opus-4-7`, `claude-opus-4-6`, and `claude-sonnet-4-6`.
...
## Continue with an SDK

Use the [OpenAI and Anthropic SDK guide](/guides/openai-anthropic-sdks) to run protocol-native Python or TypeScript clients with the same Capriole AI API key.

After the first request, use the [API keys and usage guide](/guides/api-keys-usage) to verify the key, model, status, and charged-token record.
