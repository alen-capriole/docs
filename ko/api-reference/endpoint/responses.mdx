...
`POST /v1/responses`는 `openai-latest`, `openai/gpt-6-astra`, `openai/gpt-5.6-terra`, `openai/gpt-5.6-luna`, `openai/gpt-5.5`를 받습니다. Capriole AI가 권장 OpenAI Responses 플래그십 모델을 선택하게 하려면 `openai-latest`를 사용하세요.

`gpt-6-astra`처럼 `openai/` 접두사가 없는 네이티브 OpenAI ID도 보낼 수 있습니다. 네이티브 ID와 접두사가 붙은 ID는 라우팅, 요금 계산, 할당량 차감에 동일한 모델 항목을 사용합니다.
