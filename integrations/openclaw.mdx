...
openclaw config set models.providers.capriole-ai-claude.api \
  '"anthropic-messages"' \
  --strict-json
```
...
OpenClaw currently supports GPT 5.5 and Opus 4.7 through Capriole AI.
...
Confirm that `models.providers.capriole-ai-claude.api` is set to `"anthropic-messages"` and the Claude `baseUrl` is `https://api.caprioletech.com` without `/v1`.
...
Run `openclaw models list`, then confirm the selected model and any `agents.defaults.models` allowlist include `capriole-ai-openai/openai/gpt-5.5` or `capriole-ai-claude/claude-opus-4-7`.
