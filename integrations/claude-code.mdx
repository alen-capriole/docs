...
- This setup uses `high` as the documented default effort level. On Opus 4.7, `xhigh` is also a persistent `effortLevel` option, while `max` is a session-level override and can spend more thinking tokens.
