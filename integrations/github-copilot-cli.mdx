...
GitHub Copilot CLI 1.0.82: local file-read and final-answer checks pass with `anthropic/claude-fable-5-1`, `google/gemini-3.8-flash`, and `zai/glm-5.3-flash`. `claude-latest` resolves to Fable 5.1. Previous concrete model IDs remain available.

The new Chat Completions checks use `COPILOT_PROVIDER_MODEL_ID="gpt-5.4"`; Fable 5.1 also passes through Anthropic Messages with `claude-latest`.
...
`COPILOT_PROVIDER_MODEL_ID` is the client-side model identifier; `COPILOT_PROVIDER_WIRE_MODEL` is the model ID sent to Capriole AI. Use `gpt-6-astra` with `openai-latest` for GPT-6 Astra. Copilot CLI 1.0.82 accepts this custom ID but has no built-in Astra profile, so client configuration and token limits use its defaults unless explicitly overridden. See `copilot help providers`.

Local checks with this Astra client ID complete a file read on Chat Completions. Responses returns the correct file contents when given an absolute path, but also reproduces incorrect relative-path resolution and long waits. Changing the client ID is not a fix for those task-level issues; validate your own workflow before rollout.
...
GitHub Copilot CLI 1.0.75 completes streamed shell-tool workflows with `anthropic/claude-fable-5`, `xai/grok-4.5`, `zai/glm-5.2`, and `moonshot/kimi-k3` through this Chat Completions path.
...
Set both `COPILOT_PROVIDER_MODEL_ID` and `COPILOT_PROVIDER_WIRE_MODEL`. For GPT-6 Astra, use `gpt-6-astra` and `openai-latest`, respectively.
