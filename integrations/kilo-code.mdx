...
Kilo Code 7.3.16: local file-read and final-answer checks pass with `anthropic/claude-fable-5-1`, `google/gemini-3.8-flash`, and `zai/glm-5.3-flash`. `claude-latest` resolves to Fable 5.1. Previous concrete model IDs remain available.

The Gemini 3.8 Flash checks use `"tool_call": true`; the Gemini 3.5 Flash setting below remains `"tool_call": false`.
...
The Kilo docs use the same family order as the Capriole AI model picker: OpenAI, Anthropic, then Google. Public API integrations exclude web-chat-only presets such as Fable 5 Thinking and Opus 5 Thinking.
...
- Kilo Code 7.3.16 completes a streamed bash tool call and the follow-up assistant turn with Kimi K3.
