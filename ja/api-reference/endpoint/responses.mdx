...
`POST /v1/responses` は `openai-latest`、`openai/gpt-6-astra`、`openai/gpt-5.6-terra`、`openai/gpt-5.6-luna`、`openai/gpt-5.5` を受け付けます。推奨する旗艦 OpenAI Responses モデルを Capriole AI に選ばせるには、`openai-latest` を使ってください。

`gpt-6-astra` のように、`openai/` プレフィックスのないネイティブ OpenAI ID も送信できます。ネイティブ ID とプレフィックス付き ID は、ルーティング、料金、利用枠の計算に同じモデルレコードを使います。
