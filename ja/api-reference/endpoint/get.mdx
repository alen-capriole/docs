...
| `openai/gpt-6-astra` などの具体的なモデル ID | 特定のモデル バージョンに固定します |
...
Anthropic Messages エンドポイントは、`claude-fable-5-1`、`claude-fable-5`、`claude-opus-5`、`claude-opus-4-8`、`claude-opus-4-7`、`claude-opus-4-6`、`claude-sonnet-4-6` などのプレフィックスなしの具体的な Claude Messages ID も受け付けます。

`POST /v1/messages` と `POST /v1/messages/count_tokens` では、`claude-latest` は `claude-fable-5-1` に解決されます。これは Chat と Chat Completions の latest エイリアスと一致します。
