---
title: "Use Files, Image Generation and Web Search in Capriole AI"
sidebarTitle: "Files, Images and Web Search"
description: "Generate images with GPT Image 2.5 for free, upload files, and use web search in Browser Chat. Learn the sign-in and Server Storage requirements."
---
...
The same workspace can continue the task with GPT-6 Astra, Claude Fable 5.1, or Gemini 3.1 Pro without copying the material into separate products. Premium costs USD 8 per month and makes browser chat unlimited across the supported Premium model set.
...
1. Sign in and use **Server Storage**: in **Account** → **Settings**, leave **Enable local storage** and **Enable No Storage** off.
2. Open the model selector, choose the **Image** tab, and select **GPT Image 2.5**. Describe the image you want.
3. Choose an aspect ratio, **Resolution**, and a batch of **1–4 images**, then send. Supported ratios are **1:1, 3:2, 2:3, 4:3, 3:4, 16:9, and 9:16**; resolutions are **1K, 2K, and 4K**.
4. Wait for completion in the thread. You can reopen the saved conversation, view each completed image, inspect **Info**, or download it.

You can have up to **four active images** per account. In Text mode, ask the Text Agent to create an image instead of switching tabs; it uses the same image-generation service. The browser currently supports text-to-image generation, not editing existing images, masks, or reference-image inputs. Only completed images are shown as results.

For the model's official capabilities, see [OpenAI's model documentation](https://developers.openai.com/api/docs/models/gpt-image-2.5-sunburst). The features available in Capriole AI are described above.
